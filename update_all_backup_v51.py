#!/usr/bin/env python3
"""
MAGI update_all.py v5.1 — 통합 현황판 (아카이브 스타일) + 거래 로그 동기화
설계: 소니 | 구현: 별이
...
v5.1 변경: 거래로그 중복 방지를 로컬 원장(synced_orders.json) 기반으로 근본 개선
...
def create_trade_record(db_id, trade_data, strategy, token):
    ...
    if strategy == "kai" and "ema_state" in extras:
        props["EMA상태"] = {"select": {"name": extras["ema_state"]}}
    elif strategy == "jet":
        if "squeeze" in extras:
            props["스퀴즈여부"] = {"select": {"name": extras["squeeze"]}}
        if "time_stop" in extras:
            props["시간손절여부"] = {"select": {"name": extras["time_stop"]}}
    elif strategy == "boomerang" and "rsi_direction" in extras:
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("📊 update_all.py v5.1 실행 시작")
    try:
        # 설정 로드
        ...
        logger.info(f"📊 update_all.py v5.1 완료 ({elapsed:.1f}초)")
